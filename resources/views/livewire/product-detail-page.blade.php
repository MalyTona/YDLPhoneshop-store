<div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-4 sm:py-8">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <nav class="flex mb-4 sm:mb-8" aria-label="Breadcrumb">
      <ol class="inline-flex items-center space-x-1 md:space-x-3 text-xs sm:text-sm">
        <li class="inline-flex items-center">
          <a wire:navigate href="/"
            class="inline-flex items-center font-medium text-gray-700 hover:text-amber-600 dark:text-gray-400 dark:hover:text-white">
        ...
        <li>
          <div class="flex items-center">
            ...
            <a wire:navigate href="/products"
              class="ml-1 font-medium text-gray-700 hover:text-amber-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">ផលិតផល</a>
          </div>
        </li>
        @if($product->category)
      <li class="hidden sm:block">
        ...
        <a wire:navigate href="/products?selected_categories[0]={{ $product->category->id }}"
          class="ml-1 font-medium text-gray-700 hover:text-amber-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">{{ $product->category->name }}</a>
        </div>
      </li>
    @endif
        <li aria-current="page">
          <div class="flex items-center">
            ...
            <span
              class="ml-1 font-medium text-gray-500 md:ml-2 dark:text-gray-400 truncate">{{ Str::limit($product->name, 20) }}</span>
          </div>
        </li>
      </ol>
    </nav>
    ...
    <div
      class="mb-4 sm:mb-6 bg-red-50 border border-red-200 rounded-lg p-3 sm:p-4 dark:bg-red-900/20 dark:border-red-800">
      <div class="flex">
      <svg class="w-4 h-4 sm:w-5 sm:h-5 text-red-400 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
        <path fill-rule="evenodd"
        d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L10 10.414l1.293-1.293a1 1 0 001.414-1.414L10 8.586 8.707 7.293z"
        clip-rule="evenodd" />
      </svg>
      <div class="ml-3">
  ...
  @endif
    ...
    @if($product->on_sale)
    <div
      class="mb-4 sm:mb-6 bg-amber-50 border border-amber-200 rounded-lg p-3 sm:p-4 dark:bg-amber-900/20 dark:border-amber-800">
      <div class="flex">
  ...
  @endif
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg sm:shadow-xl overflow-hidden">
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-8 p-4 sm:p-6 lg:p-8">
        ...
        <div class="space-y-3 sm:space-y-3 lg:col-span-5" x-data="{ 
          ...
          <div class="relative">
            ...
            <div
              class="relative bg-gray-100 dark:bg-gray-700 rounded-lg sm:rounded-xl overflow-hidden group cursor-zoom-in"
              @click="isZoomed = true" style="aspect-ratio: 1/1;">
              ...
              @if($product->on_sale)
          <div
          class="absolute top-2 sm:top-4 left-2 sm:left-4 bg-red-500 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-bold">
          SALE
          </div>
        @endif
          ...
          <div
          class="absolute top-2 sm:top-4 {{ $product->on_sale ? 'left-14 sm:left-20' : 'left-2 sm:left-4' }} bg-amber-500 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-bold">
          ⭐ FEATURED
          </div>
        @endif
              ...
              <!-- Zoom Icon -->
              <div
                class="absolute top-2 sm:top-4 right-2 sm:right-4 bg-white/80 dark:bg-gray-800/80 rounded-full p-1.5 sm:p-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <svg class="w-4 h-4 sm:w-5 sm:h-5 text-gray-600 dark:text-gray-300" fill="none" stroke="currentColor"
                  viewBox="0 0 24 24">
              ...
              <template x-if="images.length > 1">
                <div
                  class="absolute inset-0 flex items-center justify-between px-2 sm:px-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  <button
                    ...
                    <svg class="w-4 h-4 sm:w-5 sm:h-5 text-gray-600 dark:text-gray-300" fill="none"
              ...
              <template x-if="images.length > 1">
                <div
                  class="absolute bottom-2 sm:bottom-4 right-2 sm:right-4 bg-black/70 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium">
      ...
      @endif
        ...
        <div class="space-y-4 sm:space-y-6 lg:col-span-5">
          ...
          <div class="flex items-center text-xs sm:text-sm text-gray-500 dark:text-gray-400">
            <span>ប្រភេទ:</span>
            <a wire:navigate href="/products?selected_categories[0]={{ $product->category->id }}"
            class="ml-1 text-amber-600 dark:text-amber-400 font-medium hover:underline">
        ...
        @endif
          ...
          <div class="flex items-center text-xs sm:text-sm text-gray-500 dark:text-gray-400">
            <span>ម៉ាក:</span>
            <a wire:navigate href="/products?selected_brands[0]={{$product->brand->id}}"
            class="ml-1 text-amber-600 dark:text-amber-400 font-medium hover:underline">
            {{ $product->brand->name }}
            </a>
          </div>
        @endif
          ...
          <span
            class="bg-red-100 text-red-800 text-xs sm:text-sm font-medium px-2 sm:px-2.5 py-0.5 rounded dark:bg-red-900 dark:text-red-300">
            -17% OFF
          </span>
          </div>
        @endif
      ...
      @endif
        ...
        <div class="flex items-center space-x-3">
          ...
          <input wire:model='quantity' type="number" min="1" max="99"
      ...
      @else
        <div class="text-center py-6 sm:py-8">
          ...
          <button
          class="bg-gray-300 dark:bg-gray-600 text-gray-500 dark:text-gray-400 font-semibold py-3 sm:py-4 px-4 sm:px-6 rounded-lg sm:rounded-xl cursor-not-allowed text-sm sm:text-base">
          មិនអាចទិញបាន
          </button>
        </div>
      @endif
        ...
        @endif
              <div>
                ...
                <span
                  class="font-medium ml-1 {{ $product->in_stock ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
    ...
    <div
      class="fixed bottom-4 right-4 bg-green-500 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg shadow-lg z-50 text-sm sm:text-base max-w-xs sm:max-w-sm"
  ...
  @endif
    ...
    <div
      class="fixed bottom-4 right-4 bg-red-500 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg shadow-lg z-50 text-sm sm:text-base max-w-xs sm:max-w-sm"
      x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)">
      {{ session('error') }}
    </div>
  @endif
