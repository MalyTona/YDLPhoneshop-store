    ...
    <div class="w-full max-w-7xl py-10 px-4 sm:px-6 lg:px-8 mx-auto">
            ...
            <div
                ...
                <div class="text-center">
                    @if(!empty($search))
                        ...
                        <div class="mt-2 mb-4 h-1 w-32 mx-auto bg-gradient-to-r from-amber-400 to-blue-500 rounded-full">
                        </div>
        ...
        @if(!empty($search))
            ...
            <div class="lg:hidden mb-6 space-y-4 max-w-4xl mx-auto">
                <!-- Categories Filter -->
                <div
                    class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-4 mx-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mx-2">
                    ...
                    <div
                        class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-4">
                ...
                <div class="lg:col-span-1 space-y-6">
                    ...
                    <div
                        ...
                        <div class="space-y-3">
                            @foreach ($categories as $category)
                                <label class="flex items-center group cursor-pointer" wire:key="{{ $category->id }}">
                                    <input type="checkbox" id="{{ $category->slug }}_desktop"
                    ...
                    <div
                        ...
                        <div class="w-16 h-1 bg-gradient-to-r from-amber-400 to-blue-500 rounded-full mb-6"></div>
                        <div class="space-y-4">
                            <div class="font-semibold text-center dark:text-white">{{Number::currency($price_range, 'USD')}}
                            </div>
                            <input wire:model.live='price_range' type="range"
                                class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700 slider"
                                max="2000" min="50" step="10">
                    ...
                    @if($products->count() > 0)
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                            @foreach ($products as $product)
                                <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden group hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
                                    wire:key="{{ $product->id }}">
                                ...
                                <div class="p-3">
                                    ...
                                    @if($product->in_stock && $product->is_active)
                                        <button wire:click.prevent="addToCart({{ $product->id }})"
                                            class="w-full bg-amber-600 hover:bg-amber-700 text-white font-medium py-2 px-3 rounded-lg transition-colors duration-200 flex items-center justify-center space-x-1">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-2.5 5M7 13l2.5 5m0 0h8m-8 0a2 2 0 100 4 2 2 0 000-4zm8 0a2 2 0 100 4 2 2 0 000-4z" />
                                            </svg>
                                            <span wire:loading.remove wire:target='addToCart({{ $product->id }})'
                                                class="text-xs">ទិញ</span>
                                            <span wire:loading wire:target='addToCart({{ $product->id }})'>កំពុងទិញ...</span>
                                        </button>
                                    @else
                                        <button
                                            class="w-full bg-gray-300 dark:bg-gray-600 text-gray-500 dark:text-gray-400 font-medium py-2 px-3 rounded-lg cursor-not-allowed">
